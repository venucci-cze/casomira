...
from tkinter import ttk, messagebox  # Pro lepší tabulku
import requests
import time
...
def _update_gui_from_thread():
    ...
    if team_a_label:
        team_a_label.configure(text=str(time_a))
    if team_b_label:
        team_b_label.configure(text=str(time_b))
    if status_label:
        status_label.configure(text="Hotovo")
...
def update_display():
    ...
    global status_label
    if status_label:
        status_label.configure(text="čtení")
...
def check_and_apply_update():
    """
    Kontroluje dostupnou aktualizaci a zavolá updater.exe.
    """
    try:
        update_info = check_for_updates()
        
        if not update_info:
            messagebox.showinfo("Aktualizace", "Máš již nejnovější verzi!")
            return
        ...
        if response:
            ...
            # Spusť updater.exe (nebo updater.py fallback)
            try:
                ...
                print(f"❌ Chyba spuštění updateru: {e}")
                messagebox.showerror("Chyba", f"Nelze spustit updater:\n{e}")
                return
            
            # Vypni casomira
            print("Vypínám aplikaci...")
            root.quit()
            sys.exit(0)
    
    except Exception as e:
        print(f"❌ Chyba: {e}")
        messagebox.showerror("Chyba", f"Chyba při kontrole aktualizací:\n{e}")
...
def main_app():
    ...
    tree.column("Tým A", width=100, anchor='w')
    tree.column("Tým B", width=100, anchor='w')
